...
def calc_answer(filepath, isbbq = False):
    ...
    if isbbq:
        ...
        tt_num = 0
        for itm in data:
            input = itm["raw_response"].split("assistant\n")[0]
            output = itm["raw_response"].split("assistant\n")[-1]
...
if __name__ == "__main__":
    ...
    for model in ["llama3.1-8b-instruct"]:
        ...
        for dataset in ["bbq", "crowdspairs", "genMO", "stereoset", "winoqueer"]:
            ...
            print(model, dataset)
            pred_file = f"<REPO_ROOT>/results/original.unbiased_messages/{model}/{dataset}.original.{model}.luo_method_enhanced.jsonl"
            print(pred_file)
